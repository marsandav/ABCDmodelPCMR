%% Definition of the system with embedded plano-concave microresonator (PCMR)

% Illumination beam 
spotsize = 38e-6;                                   % Spot size [m] 
w00 = spotsize/2;                                   % Beam waist [m] 
dlambda = 0.001;                                    % Wavelength step [nm] 
lambda_ini = 1550;                                  % Initial wavelength [nm]
lambda_end = 1551;                                  % Final wavelength [nm]
...
ROC = 2.54e-3;                                      % ROC of second mirror [m]
mu = 1;                                             % Absorption coefficient of the cavity (m^-1)
...
% Reflection from a spherical mirror (M2)
A=1;                            B=0;	
C=-2/ROC;                       D=1;
M_refl2=[A B;C D];
...
while (m<m_max) && ((m < 100) || (ch > ch_tol))     
    ...
    b=exp(-mu*2*m*L);                                   % Loss due to absorption for each round-trip
